...
ts = msprime.sim_ancestry(sequence_length=100, samples={"pop1": 10, "pop2": 10, "Ghost":10}, demography=demography, random_seed=352)

#Simulate mutations along the tree sequence simulated by msprime above
mts = msprime.sim_mutations(ts,model="binary", rate=0.01, random_seed=352)
...
with open("model8_replicate2.vcf","w") as vcf_file:
