...
ts = msprime.sim_ancestry(sequence_length=100, samples={"pop1": 10, "pop2": 10, "Ghost":10}, demography=demography, random_seed=959)

#Simulate mutations along the tree sequence simulated by msprime above
mts = msprime.sim_mutations(ts, rate=0.01, random_seed=959, model="binary")
...
with open("model5_replicate6.vcf","w") as vcf_file:
